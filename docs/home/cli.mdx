...
Every executed command is recorded by a hidden recorder. The `history`
builtin shows the calling session's commands (GNU bash semantics,
`history -c` clears only that session's view), and `/.bash_history`
renders the GNU histfile across all sessions, readable with the
