...
commands like `ls`, `cat`, `grep`, `find`, and `search`.
...
The `search` command calls Chroma's native vector query API. Use it when
semantic similarity matters more than exact text matching.

```bash
search "how do I get started" /knowledge/
search --top-k 5 "billing policy" /knowledge/policies/
search --top-k 3 "API authentication" /knowledge/guides/quickstart.md
```
...
- [`chroma.py`](https://github.com/StruktoAI/mirage/blob/main/examples/python/chroma/chroma.py) — command workflow with `ls`, `tree`, `find`, `cat`, `head`, `tail`, `grep`, and `search`.
...
| `search` | Vector retrieval through Chroma |
