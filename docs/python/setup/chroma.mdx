...
If `path` is set, Mirage uses Chroma's `PersistentClient`. Otherwise it uses
`HttpClient` with `host`, `port`, and `ssl`.
...
| `path` | No | `None` | Local persistent Chroma directory |
...
```bash
CHROMA_PATH=/absolute/path/to/chroma
CHROMA_COLLECTION=knowledge
```
